...
const GodQuotesPage = () => {
  const [godQuotes, setGodQuotes] = useState([]);
  const [fadeIn, setFadeIn] = useState(false);

  useEffect(() => {
    async function loadGodQuotes() {
      try {
        const response = await fetch('/GodQuotes.json');
        const data = await response.json();
        const filteredQuotes = data.filter((q) => q.tags.includes('god'));
        setGodQuotes(filteredQuotes);
        setTimeout(() => setFadeIn(true), 100);
      } catch (error) {
        console.error('Error loading god quotes:', error);
      }
    }
