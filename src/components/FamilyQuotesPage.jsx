...
const FamilyQuotesPage = () => {
  const [familyQuotes, setFamilyQuotes] = useState([]);
  const [fadeIn, setFadeIn] = useState(false);

  useEffect(() => {
    async function loadFamilyQuotes() {
      try {
        const response = await fetch('/familyQuotes.json');
        const data = await response.json();
        const filteredQuotes = data.filter((q) => q.tags.includes('family'));
        setFamilyQuotes(filteredQuotes);
        setTimeout(() => setFadeIn(true), 100);
      } catch (error) {
        console.error('Error loading family quotes:', error);
      }
    }
...
export default FamilyQuotesPage;