...
const SuccessQuotesPage = () => {
  const [successQuotes, setSuccessQuotes] = useState([]);
  const [fadeIn, setFadeIn] = useState(false);

  useEffect(() => {
    async function loadSuccessQuotes() {
      try {
        const response = await fetch('/SuccessQuotes.json');
        const data = await response.json();
        const filteredQuotes = data.filter((q) => q.tags.includes('success'));
        setSuccessQuotes(filteredQuotes);
        setTimeout(() => setFadeIn(true), 100);
      } catch (error) {
        console.error('Error loading success quotes:', error);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-100 dark:from-gray-900 dark:to-purple-950 text-gray-900 dark:text-white relative overflow-hidden">
                ...
                <Card className="backdrop-blur-lg bg-white/70 dark:bg-gray-800/40 border border-white/20 dark:border-gray-700/30 shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-102 overflow-hidden rounded-2xl">
                  <CardContent className="p-0">
                    <div className="relative p-8 md:p-10">
                      <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-100/40 to-purple-100/40 dark:from-blue-900/20 dark:to-purple-900/20 z-0"></div>
                      <div className="relative z-10">
                        <blockquote className="font-serif italic text-xl md:text-2xl text-center leading-relaxed text-gray-800 dark:text-gray-100 transition-colors duration-300 mb-6">
                          "{quote.quote}"
                          <div className="mt-2 w-16 h-1 mx-auto bg-gradient-to-r from-blue-400 to-purple-400 rounded-full"></div>
                        </blockquote>
...
export default SuccessQuotesPage;