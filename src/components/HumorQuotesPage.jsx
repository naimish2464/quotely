...
const HumorQuotesPage = () => {
  const [humorQuotes, setHumorQuotes] = useState([]);
  ...
  useEffect(() => {
    async function loadHumorQuotes() {
      try {
        setLoading(true);
        const response = await fetch('/humorQuotes.json');
        const data = await response.json();
        const filteredQuotes = data.filter((q) => q.tags.includes('humor'));
        setHumorQuotes(filteredQuotes);
        setLoading(false);
        setTimeout(() => setFadeIn(true), 100);
      } catch (error) {
        console.error('Error loading humor quotes:', error);
        setLoading(false);
      }
    }
  ...
  const handleCopy = (quote) => {
    ...
    const currentQuotes = humorQuotes.slice(indexOfFirstQuote, indexOfLastQuote);
    const totalPages = Math.ceil(humorQuotes.length / quotesPerPage);
...
export default HumorQuotesPage;