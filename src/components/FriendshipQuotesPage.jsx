...
const FriendshipQuotesPage = () => {
  const [friendshipQuotes, setFriendshipQuotes] = useState(null);
  const [fadeIn, setFadeIn] = useState(false);

  useEffect(() => {
    async function loadFriendshipQuotes() {
      try {
        const response = await fetch('/friendship_quotes.json');
        const data = await response.json();
        const filteredQuotes = data.filter((q) => q.tags.includes('Friendship'));
        setFriendshipQuotes(filteredQuotes);
        setTimeout(() => setFadeIn(true), 100);
      } catch (error) {
        console.error('Error loading friendship quotes:', error);
      }
    }

    loadFriendshipQuotes();
  }, []);
