...
const SwamiVivekanandaQuotesPage = () => {
  const [swamiVivekanandaQuotes, setSwamiVivekanandaQuotes] = useState([]);
  const [fadeIn, setFadeIn] = useState(false);

  useEffect(() => {
    async function loadSwamiVivekanandaQuotes() {
      try {
        const response = await fetch('/swamiVivekananda.json');
        const data = await response.json();
        setSwamiVivekanandaQuotes(data);
        setTimeout(() => setFadeIn(true), 100);
      } catch (error) {
        console.error('Error loading Swami Vivekananda quotes:', error);
      }
    }

    loadSwamiVivekanandaQuotes();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-800 text-gray-900 dark:text-white relative overflow-hidden">
      <main className="relative z-10">
        <div className="container mx-auto px-4 py-12">
          <h1 className="text-4xl md:text-5xl font-bold text-center mb-12 tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-gray-600 to-gray-700 dark:from-gray-400 dark:to-gray-500 transition-all duration-500">
            Swami Vivekananda Quotes
          </h1>
          <div className="flex flex-col items-center space-y-16">
            {swamiVivekanandaQuotes.map((quote, index) => (
              <div
                key={index}
                ...
                <Card className="backdrop-blur-lg bg-white/70 dark:bg-gray-800/40 border border-white/20 dark:border-gray-700/30 shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-102 overflow-hidden rounded-2xl">
                  <CardContent className="p-0">
                    <div className="relative p-8 md:p-10">
                      <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-gray-100/40 to-gray-200/40 dark:from-gray-900/20 dark:to-gray-800/20 z-0"></div>
                      <div className="relative z-10">
                        <blockquote className="font-serif italic text-xl md:text-2xl text-center leading-relaxed text-gray-800 dark:text-gray-100 transition-colors duration-300 mb-6">
                          "{quote.quote}"
                          <div className="mt-2 w-16 h-1 mx-auto bg-gradient-to-r from-gray-400 to-gray-500 rounded-full"></div>
                        </blockquote>
                    ...
                    <div className="px-8 py-4 border-t border-gray-200/50 dark:border-gray-700/50 flex justify-between items-center bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm">
                      <div className="flex gap-4">
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 rounded-full text-gray-600 hover:text-gray-600 dark:text-gray-400 dark:hover:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-all duration-300"
                      ...
                      <div className="flex gap-4">
                        <button
                          onClick={() => handleWhatsAppShare(quote.quote, quote.author)}
                        ...
                        <button
                          onClick={() => handleInstagramShare(quote.quote, quote.author)}
...
export default SwamiVivekanandaQuotesPage;