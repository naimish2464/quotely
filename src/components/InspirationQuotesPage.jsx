...
const InspirationQuotesPage = () => {
  const [inspirationQuotes, setInspirationQuotes] = useState([]);
  const [fadeIn, setFadeIn] = useState(false);

  useEffect(() => {
    async function loadInspirationQuotes() {
      try {
        const response = await fetch('/InspirationQuotes.json');
        const data = await response.json();
        const filteredQuotes = data.filter((q) => q.tags.includes('inspiration'));
        setInspirationQuotes(filteredQuotes);
        setTimeout(() => setFadeIn(true), 100);
      } catch (error) {
        console.error('Error loading inspiration quotes:', error);
      }
    }
...
export default InspirationQuotesPage;